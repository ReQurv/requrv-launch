...
Tauri 2 + Nuxt 4 desktop app that launches AI coding agents (OpenCode, Codex, Claude Code) pre-configured for the AI Hive gateway. Frontend in `app/`, Rust backend in `src-tauri/`.
...
- Claude Code is terminal-only: no persistent config, it's launched with env vars (`ANTHROPIC_BASE_URL` = `HIVE_ANTHROPIC_BASE_URL` — note it's the gateway **without** the trailing `/v1`, because the client appends `/v1/messages` — plus `ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL`, `CLAUDE_CODE_MAX_CONTEXT_TOKENS=128000`) and is only launched after probing the gateway's `/messages` endpoint (Anthropic Messages API; the gateway must also accept `role:"system"` in `messages[]`, which Claude Code v2.x sends).
